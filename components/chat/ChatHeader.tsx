...
export function ChatHeader({ partnerName, partnerPhoto, backRoute, isChatUnlocked }: ChatHeaderProps) {
  ...
  return (
    <div className="px-8 pt-4 pb-0 sticky top-0 z-10 bg-[#FAF9F7]/80 backdrop-blur-xl">
      <div className="flex items-center gap-3 py-4">
        <button onClick={() => router.push(backRoute)} className="btn-ghost p-2 -ml-2">
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div className="flex items-center gap-3 flex-1">
          <div className="w-10 h-10 rounded-full flex items-center justify-center overflow-hidden bg-[#F0EDE9]">
            {partnerPhoto ? (
