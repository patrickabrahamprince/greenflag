...
export function OtpVerificationForm({
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <button type="button" onClick={onChangeNumber} className="flex items-center gap-1.5 text-muted text-sm hover:text-white transition-colors mb-2">
      ...
      <button type="button" onClick={onResendOtp} disabled={loading} className="text-xs text-muted hover:text-white transition-colors w-full text-center font-thin">
        Resend OTP
      </button>
    </form>
  );
