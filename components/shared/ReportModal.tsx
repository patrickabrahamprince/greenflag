          ...
          <div>
            ...
            <div className="space-y-2">
              {['Fake profile', 'Inappropriate content', 'Harassment', 'Other'].map((r) => (
                <label
                  key={r}
                  className="flex items-center gap-3 p-3 rounded-lg border border-[#2A2A2A] cursor-pointer transition-colors hover:border-[#D4AF37]/40"
                >
                  <span
                    className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors ${
                      reason === r ? 'border-[#D4AF37]' : 'border-[#9DA0A6]'
                    }`}
                  >
                    {reason === r && <span className="w-2 h-2 rounded-full bg-[#D4AF37]" />}
                  </span>
