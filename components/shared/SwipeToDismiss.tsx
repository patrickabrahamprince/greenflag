...
// follows the finger. Direction: swipe LEFT reveals pin on the right (and
// just snaps back -- pinning doesn't remove anything), swipe RIGHT
// reveals delete on the left (and removes the card).
//
...
export function SwipeToDismiss({ onDelete, onPin, pinned, children }: SwipeToDismissProps) {
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (Math.abs(dx) > Math.abs(dy)) {
      // Dragging left without a pin handler has nothing to reveal.
  ...
  const handleTouchEnd = () => {
    setDragging(false);
    if (dragX <= -ACTION_THRESHOLD && onPin) {
      onPin();
      setDragX(0);
    } else if (dragX >= ACTION_THRESHOLD) {
      setRemoving(true);
      setDragX(400);
  ...
  return (
    <div className="relative overflow-hidden">
      <div className="absolute inset-0 flex items-center">
        <div
          className="ml-3 w-9 h-9 rounded-full bg-red-500/20 flex items-center justify-center shrink-0"
        ...
        {onPin && (
          <div
            className="mr-3 w-9 h-9 rounded-full bg-gold/20 flex items-center justify-center shrink-0"
