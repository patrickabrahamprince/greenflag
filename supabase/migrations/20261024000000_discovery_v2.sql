-- Discovery v2: symmetrical matching for both genders

BEGIN;

-- 0. Enable PostGIS for distance calculations
CREATE EXTENSION IF NOT EXISTS postgis WITH SCHEMA extensions;

-- 1.1 Ensure all needed columns exist
...
-- 1.2 Ensure interests / looking_for_interests columns (the unified matching cols)
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS interests TEXT[] DEFAULT '{}',
  ADD COLUMN IF NOT EXISTS looking_for_interests TEXT[] DEFAULT '{}';

-- 1.3 Blocked pairs table
...
-- 1.5 Unified matching function for both genders
...
DROP POLICY IF EXISTS "users_own_blocks" ON blocked_pairs;
CREATE POLICY "users_own_blocks" ON blocked_pairs
FOR ALL USING (auth.uid() = host_id OR auth.uid() = guest_id);
