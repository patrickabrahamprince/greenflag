...
export async function GET() {
  try {
    ...
    const [totalUsersRes, hostsRes, guestsRes, activeConnsRes, pendingModRes, revenueRes, todayConnsRes, activityRes] = await Promise.all([
      ...
      supabase.from('connections').select('*', { count: 'exact', head: true }).gte('created_at', new Date().toISOString().slice(0, 10)),
    ...
    const activeConnections = activeConnsRes.count || 0;
    const pendingModeration = pendingModRes.count || 0;
    const revenue = (revenueRes.data || []).reduce((sum, r) => sum + (r.amount || 0), 0);
    const connectedPairsToday = todayConnsRes.count || 0;
