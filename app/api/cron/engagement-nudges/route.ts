...
async function nudgeUnseenStandardBegins(admin: Admin): Promise<number> {
  // She hasn't opened it yet -- a curiosity-driven follow-up beats
  // repeating the same transactional copy she already ignored once.
  ...
  for (const row of rows ?? []) {
    ...
    const connectionId = (row.data as Record<string, unknown> | null)?.connectionId;
    await sendNotification({
      supabase: admin,
      user_id: row.user_id,
      title: "Still there? 👀",
      body: "Someone's been waiting on you to set your Standard.",
...
async function nudgeDormantUsers(admin: Admin): Promise<number> {
  ...
  for (const row of rows ?? []) {
    ...
    await sendNotification({
      ...
      body: "You're missing profiles that match your Standard. Come take a look.",
...
async function nudgeIdleCoins(admin: Admin): Promise<number> {
  ...
  const { data: recentSpends } = await admin
  ...
  for (const wallet of wallets) {
    ...
    await sendNotification({
      ...
      body: `You have ${wallet.balance} coins sitting unused. Go meet someone's Standard today.`,
...
async function nudgeEarlyReviews(admin: Admin): Promise<number> {
  ...
  const { data: rows } = await admin
    .from('matches')
    .select('id, user2_id, review_deadline')
  ...
  for (const row of rows ?? []) {
    ...
    await sendNotification({
      ...
      body: "Don't keep him hanging -- review his submission now.",
...
async function nudgeIncompleteProfiles(admin: Admin): Promise<number> {
  ...
  const { data: rows } = await admin
    .from('profiles')
    .select('id, bio, photos')
    .lt('created_at', dayAgo)
    .eq('is_banned', false);

  let sent = 0;
  for (const row of rows ?? []) {
    const incomplete = !row.bio || !row.photos || row.photos.length < 2;
    ...
    await sendNotification({
      ...
      body: 'Profiles with a bio and photos get noticed way more. Finish yours in under a minute.',
